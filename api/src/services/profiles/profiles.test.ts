...
describe('profiles', () => {
  ...
  scenario('creates a profile', async () => {
    const result = await createProfile({
      input: {
        ...
        lastName: 'String',
        bio: 'String',
        updatedAt: '2023-12-02T23:17:08.885Z',
      },
    })
    ...
    expect(result.updatedAt).toEqual(new Date('2023-12-02T23:17:08.885Z'))
  })
  ...
  scenario('updates a profile', async (scenario: StandardScenario) => {
    ...
    const result = await updateProfile({
      id: original.id,
      input: { picture: 'String2' },
    })

    expect(result.picture).toEqual('String2')
  })
