...
export const Event: EventRelationResolvers = {
  DailyEveningPost: (_obj, { root }) => {
    return db.event.findUnique({ where: { id: root?.id } }).DailyEveningPost()
  },
  WeeklyPost: (_obj, { root }) => {
    return db.event.findUnique({ where: { id: root?.id } }).WeeklyPost()
  },
  QuarterlyPost: (_obj, { root }) => {
    return db.event.findUnique({ where: { id: root?.id } }).QuarterlyPost()
  },
  YearEndPost: (_obj, { root }) => {
    return db.event.findUnique({ where: { id: root?.id } }).YearEndPost()
  },
  NewYearPost: (_obj, { root }) => {
    return db.event.findUnique({ where: { id: root?.id } }).NewYearPost()
  },
}
